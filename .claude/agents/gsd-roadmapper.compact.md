...
```
Milestone identity (version + name) lives in milestone headings (`## vX.Y — [Name]`) or `## Milestones` bullets (`🚧 **vX.Y [Name]**`), never in H1. A trailing version in H1 (`# Roadmap: [Project] — [Name] (vX.Y)`) corrupts milestone-name extraction (#4134). `/home/ayushz/storage/CST studies/all projects/Align/.claude/gsd-core/templates/roadmap.md` is the canonical shape.
...
```
Full template: `/home/ayushz/storage/CST studies/all projects/Align/.claude/gsd-core/templates/roadmap.md`

## STATE.md Structure
Use template from `/home/ayushz/storage/CST studies/all projects/Align/.claude/gsd-core/templates/state.md`. Key sections: Project Reference, Current Position, Performance Metrics, Accumulated Context (decisions, todos, blockers), Session Continuity.

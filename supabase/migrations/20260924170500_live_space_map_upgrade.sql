alter table public.discovery_settings add column if not exists ghost_mode boolean not null default false;

drop function if exists public.get_live_feed(float);

...
// hardcode the canonical path `@~/.hermes/gsd-core/...` (references, workflows,
...
// holding the bundled tree, so the includes resolve. In a Hermes Agent
...
// `~/.hermes/gsd-core/` is never created and every @-include resolves to
...
// `~/.hermes/gsd-core/` exists and its immutable subdirs (bin, contexts,
...
// are the directories `@~/.hermes/gsd-core/<subdir>/...` includes point into.
 ...
 *
 * Hermes Agent unpacks the package so the bundled tree sits at
...
/**
 * Ensure the canonical `~/.hermes/gsd-core/` path exposes the bundled subdirs.
 *

...
process.stdin.on('end', () => {
  ...
  try {
    ...
    // created linked worktree (plain non-GSD work, e.g. Hermes Agent plan-mode) is
    ...
    // `path` authoritative: normalization returns early for native Hermes Agent
    ...
    // or resolves them against the worktree CWD. Hermes Agent's Edit/Write require
    ...
    if (fileTopResult.status !== 0 || !fileTopResult.stdout) {
      ...
      //  - Truly outside all git repositories (e.g. ~/.hermes/plans/) → not the

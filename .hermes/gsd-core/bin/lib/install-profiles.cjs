...
// stripped at emit BEFORE any path rewrite or converter runs, so a `.hermes/` ->
 ...
 * staged directory (`rewriteStagedSkillBodies`, surface.cts): a `~/.hermes/`
...
function readInstalledCapabilitySkill(stem, registry) {
    ...
    try {
        ...
        if (node_fs_1.default.lstatSync(skillPath).isSymbolicLink())
            return null;
    ...
    try {
        ...
        if (registry) {
            ...
            for (const stem of candidateStems) {
                ...
                try {
                    if (installFs().lstatSync(destDir).isSymbolicLink())
                        continue;
                }
 ...
 *   1. applyAgentPathRewrites   (4 base ~/.hermes/ regexes; skipped for copilot/antigravity)
    ...
    try {
        ...
        for (const entry of entries) {
            ...
            if (agentCtx) {
                ...
                // Step 1: path rewrites (4 base ~/.hermes/ regexes; skipped for copilot/antigravity)

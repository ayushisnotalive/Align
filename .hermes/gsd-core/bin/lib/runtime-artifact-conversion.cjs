...
// Tool name mapping from Hermes Agent to OpenCode
...
/**
 * Convert a Hermes Agent tool name to OpenCode format
 ...
 *   Global: ~/.hermes/ → ~/.copilot/, ./.hermes/ → ./.github/
 *   Local:  ~/.hermes/ → ./.github/, ./.hermes/ → ./.github/
...
function convertClaudeToCopilotContent(content, isGlobal = false) {
    ...
    // Handle both `~/.hermes/foo` (trailing slash) and bare `~/.claude` forms in
 ...
 * by Hermes Agent as the skill identity. Emits the hyphen form (gsd-<cmd>)
 * so Hermes Agent autocomplete shows the canonical invocation form, not the
 ...
 * Hermes Agent is the native format, so minimal conversion needed —
...
function convertClaudeCommandToClaudeSkill(content, skillName, runtime = null, cmdNames = null) {
    ...
    // `name:` Hermes Agent (and Qwen/Hermes) register under (#2808). `cmdNames`
...
// `@~/.hermes/gsd-core/workflows/<stem>.md` include: line-start `@`, exact
 ...
 *
 * WHY this can't stay a static `@`-include (even a relative one): Hermes Agent
 * documents relative `@`-paths as resolving against the file *containing* the
 * import, which for a global skill is `~/.hermes/skills/gsd-<stem>/` — not
 * the project's working directory. `@./.hermes/...` would therefore always
...
function resolveSpecRootReference(body) {
    if (typeof body !== 'string' || body.length === 0)
        return body;
    if (!body.includes('@~/.hermes/gsd-core/workflows/'))
        return body;
    ...
    return body.replace(WORKFLOW_SPEC_ROOT_INCLUDE_RE, (match, stem, cr, offset) => {
        ...
        return (`To load this command's workflow spec: check for ` +
            `\`.hermes/gsd-core/workflows/${stem}.md\` relative to the current working ` +
            `directory first (project-local); if it is not there, fall back to ` +
            `\`~/.hermes/gsd-core/workflows/${stem}.md\` (the global install). If ` +
 ...
 * Path mappings depend on install mode:
 *   Global: ~/.hermes/skills/ → ~/.gemini/config/skills/ (#3738),
 *          ~/.hermes/ → ~/.gemini/antigravity/, ./.hermes/ → ./.agents/
 *   Local:  ~/.hermes/ → .agents/, ./.hermes/ → ./.agents/
...
function convertClaudeToCursorMarkdown(content) {
    ...
    // Remove Hermes Agent-specific bug workarounds before brand replacement
    converted = converted.replace(/\*\*Known Hermes Agent bug \(classifyHandoffIfNeeded\):\*\*[^\n]*\n/g, '');
    converted = converted.replace(/- \*\*classifyHandoffIfNeeded false failure:\*\*[^\n]*\n/g, '');
    // Replace "Hermes Agent" brand references with "Cursor" — #2284(b): skips
...
// without this would have silently regressed live installs: `Hermes Agent`
...
function convertClaudeToWindsurfMarkdown(content) {
    ...
    converted = converted.replace(/\*\*Known Hermes Agent bug \(classifyHandoffIfNeeded\):\*\*[^\n]*\n/g, '');
    converted = converted.replace(/- \*\*classifyHandoffIfNeeded false failure:\*\*[^\n]*\n/g, '');
    // Replace "Hermes Agent" brand references with "Windsurf" — #2284(b): skips
    // <runtime_compatibility> comparison-table content (protected region).
...
// `@~/.hermes/gsd-core/commands/gsd/<stem>.md` reference at a file that does
...
function convertClaudeCommandToWindsurfWorkflow(content, commandName) {
    ...
    return `# ${commandName}\n\n${effectiveDescription}\n\nRead and execute the GSD command at @~/.hermes/gsd-core/commands/gsd/${stem}.md end-to-end. Treat the user's message after /${commandName} as the command arguments.`;
}
// --- Augment converters ---
...
function convertClaudeToAugmentMarkdown(content) {
    ...
    converted = converted.replace(/\*\*Known Hermes Agent bug \(classifyHandoffIfNeeded\):\*\*[^\n]*\n/g, '');
    converted = converted.replace(/- \*\*classifyHandoffIfNeeded false failure:\*\*[^\n]*\n/g, '');
    // Replace "Hermes Agent" brand references with "Augment" — #2284(b): skips
    // <runtime_compatibility> comparison-table content (protected region).
...
function convertClaudeToTraeMarkdown(content) {
    ...
    // converter — for `~/.hermes/`, `$HOME/.hermes/`, AND `./.hermes/` alike —
    ...
    // left the doubled-prefix defect live for the `$HOME/.hermes/` and
    // `~/.hermes/` forms specifically (found the same way, one regression-test
    // run later). Scoped to a `.trae/` tail so it cannot also swallow the
    // unprefixed `./HERMES.md` form the very next pattern handles differently
...
function convertClaudeToCodebuddyMarkdown(content) {
    ...
    // CodeBuddy uses the same tool names as Hermes Agent (Bash, Edit, Read, Write, etc.)
...
/**
 * Convert a Hermes Agent slash-command (.md) to a CodeBuddy slash-command (.md).
 *
...
function convertClaudeToCliineMarkdown(content) {
    ...
    // Cline uses the same tool names as Hermes Agent — no tool name conversion needed
 ...
 * (.hermes/→.cline/, "Hermes Agent"→"Cline", etc.).
...
function convertClaudeCommandToClineSkill(content, skillName, _runtime = null, cmdNames = null) {
    ...
    // apply Cline-specific markdown rewrites (.hermes/→.cline/, etc.).
    const names = cmdNames || readGsdCommandNames();
    const normalizedBody = transformContentToHyphen(body, names);
...
function convertClaudeToCodexMarkdown(content) {
    ...
    // Bare/project-relative .hermes/... references (#2639). Covers strings like
    // "check `.hermes/skills/`" where there is no ~/, $HOME/, or ./ anchor.
...
GSD workflows use \`AskUserQuestion\` (Hermes Agent syntax). Translate to Codex \`request_user_input\`:
...
GSD workflows use \`Task(...)\` (Hermes Agent syntax). Translate to Codex collaboration tools:
...
function neutralizeAgentReferences(content, instructionFile) {
    ...
    // Negative lookahead avoids: Hermes Agent, Claude Opus/Sonnet/Haiku, Claude native, Claude-based
    c = c.replace(/\bClaude(?! Code| Opus| Sonnet| Haiku| native| based|-)\b(?!\.md)/g, 'the agent');
    // Replace HERMES.md with runtime-appropriate instruction file
    if (instructionFile) {
...
function convertClaudeToOpencodeFrontmatter(content, { isAgent = false, modelOverride = null, variant = null } = {}) {
    ...
    for (const line of lines) {
        ...
        // Strip model: field — OpenCode doesn't support Hermes Agent model aliases
...
function convertClaudeToKiloFrontmatter(content, { isAgent = false, modelOverride = null } = {}) {
    ...
    convertedContent = replaceRelativePathReference(convertedContent, '.hermes/skills/', '.kilo/skills/');
    convertedContent = replaceRelativePathReference(convertedContent, '.agents/skills/', '.kilo/skills/');
    convertedContent = replaceRelativePathReference(convertedContent, '.hermes/agents/', '.kilo/agents/');
    ...
    for (const line of lines) {
        ...
        // Strip model: field — Kilo doesn't support Hermes Agent model aliases
...
// Copilot tool name mapping — Hermes Agent tools to GitHub Copilot tools
...
// Tool name mapping from Hermes Agent to Gemini CLI
...
/**
 * Convert a Hermes Agent tool name to Gemini CLI format
...
/**
 * Convert a Hermes Agent tool name to GitHub Copilot format.
...
/**
 * Convert Hermes Agent agent markdown to Cursor agent format.
...
/**
 * Convert Hermes Agent agent markdown to Windsurf agent format.
...
/**
 * Convert Hermes Agent agent markdown to Augment agent format.
 ...
 *
 * `color` IS preserved: Qwen's docs list `color` under "Hermes Agent
 ...
 *
 * Body: preserved verbatim after the qwen branding rewrite (HERMES.md /
 * Hermes Agent / .hermes/ literal-substring values — descriptor-driven via
 * runtime.hostBehaviors.brandingRewrites, mirrors the qwen case in
 * _applyRuntimeRewrites). The anchored `~/.hermes/` / `$HOME/.hermes/` forms
...
function convertClaudeAgentToQwenAgent(content) {
    ...
    if (_b['HERMES.md'])
        converted = converted.replace(/CLAUDE\.md/g, _b['HERMES.md']);
    // #2284(b): skips <runtime_compatibility> comparison-table content (protected region).
    if (_b['Hermes Agent'])
        converted = applyClaudeCodeBrandSwap(converted, _b['Hermes Agent']);
    if (_b['.hermes/'])
        converted = converted.replace(/\.claude\//g, _b['.hermes/']);
    const { frontmatter, body } = extractFrontmatterAndBody(converted);
    if (!frontmatter)
        return converted;
    const name = extractFrontmatterField(frontmatter, 'name') || 'unknown';
...
/**
 * Convert a Hermes Agent agent .md for ZCode (#3384).
 *
 ...
 * not connected") whenever it is not connected, whereas Hermes Agent treats the
 ...
 *   HERMES.md    -> brandingRewrites['HERMES.md']
 *   Hermes Agent  -> brandingRewrites['Hermes Agent']  (word-boundary, \bClaude Code\b)
 *   .hermes/     -> brandingRewrites['.hermes/']
 *
...
function applyAgentBrandingRewrites(content, runtime) {
    ...
    if (_b['HERMES.md'])
        converted = converted.replace(/CLAUDE\.md/g, _b['HERMES.md']);
    if (_b['Hermes Agent'])
        converted = converted.replace(/\bClaude Code\b/g, _b['Hermes Agent']);
    if (_b['.hermes/'])
        converted = converted.replace(/\.claude\//g, _b['.hermes/']);
    return converted;
}
...
/**
 * Convert Hermes Agent agent markdown to Codex agent format.
 ...
 * non-Claude runtime, because GSD's worktree isolation was Hermes Agent's
 ...
 * context-sensitive correction layered on top of the blanket `~/.hermes/` /
 * `$HOME/.hermes/` -> pathPrefix substitution every Claude emit path
 ...
 * `node "$HOME/.hermes/gsd-core/bin/gsd-tools.cjs"`) need it, since `~` does
 * not expand inside double-quoted shell strings (#1284). But Hermes Agent's
 ...
 * automated test can verify *resolution* inside a live Hermes Agent session
 ...
 * `~` form Hermes Agent documents as expanding. A single pathPrefix string
 ...
 * every `@~/.hermes/gsd-core/…` include in a global install's workflows/
 ...
 * literal `.hermes/` segment, so it silently no-opped for any global install
 ...
 * Hermes Agent documents `@`-references as valid "anywhere in your
 * HERMES.md" (e.g. `See @README for project overview`), so anchoring to
 ...
 * like `echo "@$HOME/.hermes/x"`, where rewriting `$HOME` to `~` inside
...
function _applyRuntimeRewrites(content, runtime, pathPrefix, isGlobal = false, attribution = undefined) {
    ...
    switch (runtime) {
        ...
        // brand VALUES (HERMES.md/Hermes Agent/.hermes/ replacements) now read from
        ...
        case 'qwen': {
            ...
            // rewrites below) instead of throwing on `_b['HERMES.md']`.
            const _b = _hostBehaviors(runtime).brandingRewrites;
            if (_b) {
                content = content.replace(/CLAUDE\.md/g, _b['HERMES.md']);
                // #2284(b): skips <runtime_compatibility> comparison-table content (protected region).
                content = applyClaudeCodeBrandSwap(content, _b['Hermes Agent']);
            }
            content = content.replace(/~\/\.claude\//g, pathPrefix);
            content = content.replace(/\$HOME\/\.claude\//g, pathPrefix);
            ...
            if (_b) {
                content = content.replace(/\.claude\//g, _b['.hermes/']);
            }
            content = content.replace(/\.\/\.claude\//g, `./${dirName}/`);
        ...
        case 'zcode':
            ...
            // `${_GSD_RUNTIME_ROOT}/.hermes/…` matches none of these regexes and
 ...
 * unconditionally rewrites any bare (non-`@`-prefixed) `~/.hermes/` substring
 * in the body to the computed pathPrefix (`$HOME/.hermes/` for a global
 * install) and restores ONLY the `@`-prefixed form back to `~`
 * (`@$HOME/.hermes/` → `@~/.hermes/`). `resolveSpecRootReference`'s
 * replacement text is deliberately imperative prose containing a literal,
 * non-`@`-prefixed `~/.hermes/gsd-core/workflows/<stem>.md` — running it
 ...
 * `@~/.hermes/gsd-core/workflows/<stem>.md` include line (which survives the
...
/**
 * Apply the 4 base `~/.hermes/` path-prefix rewrites to a single agent content
 * string. Mirrors the inline agent loop in bin/install.js lines 9330-9340:
 *   ~/\.hermes/ → pathPrefix
 *   $HOME/\.hermes/ → pathPrefix
...
function applyAgentPathRewrites(content, runtime, pathPrefix) {
    ...
    // `@~/.hermes/...` include in a global Claude install shipped as `@$HOME/...`
    ...
    // fixes `@$HOME/.hermes/x` and leaves a bare `@$HOME/.claude` broken. The
...
function applyAgentFrontmatterExtensions(content, { runtime, agentName, targetDir }) {
    ...
    // #3533 (10d): 'inherit' means the effort: key must NOT exist — Hermes Agent

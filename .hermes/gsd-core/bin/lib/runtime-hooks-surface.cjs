 ...
 * 'Hermes Agent' : runtime` ternary — cosmetic (log text) only, but resolves
 * to the same 'Qwen Code' / 'Hermes Agent' values for those two runtimes.
...
function applySettingsJsonHooks(settings, opts) {
    ...
    if (hooksSurface !== 'none' && hooksSurface !== 'kimi-hooks-toml') {
        ...
        // #3981: Hermes Agent treats a timed-out hook as NON-blocking — the tool
        ...
        // write (#768): on Hermes Agent >= 2.1.259 ANY Read() deny rule makes every
        ...
        //    (#788 + #770 + #2092) ────────────────────────────────────────────────
        // Hermes Agent (since #770) and Qwen Code (since #788) both support the
        ...
        // Hermes Agent fires FileChanged when a watched file changes on disk.  Wire
        ...
        // (Hermes Agent matches by filename, not full path).  The hook exits silently
        // when the changed file is not the gsd config.
        //
        // Scoped to Hermes Agent only: Qwen Code's FileChanged support is not yet
        ...
        if (extendedEvents.includes('FileChanged')) {
            ...
            if (!alreadyHasConfigReload && node_fs_1.default.existsSync(configReloadFile) && configReloadCommand) {
                ...
                console.log(`  ${green}✓${reset} Configured FileChanged config-reload hook (Hermes Agent)`);
            }

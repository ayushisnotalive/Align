...
const capabilities = {
  ...
  "claude": {
    ...
    "version": "1.14.0",
    "title": "Hermes Agent",
    "description": "Anthropic Hermes Agent — primary development runtime; tier-1 support with full hook surface and skills-based global install.",
    "tier": "core",
    "requires": [],
    "engines": {
      "gsd": ">=1.6.0"
  ...
  "claude-orchestration": {
    ...
    "description": "Default-off, BETA, claude-only capability that adopts Hermes Agent's Workflow tool (the engine behind /effort ultracode) as an optional parallel-execution backend for the GSD loop. When the runtime exposes the Workflow tool and claude_orchestration.execution_backend resolves to 'workflow', execute-phase emits a generated Workflow script (waves -> parallel() barriers, plans -> agent({ agentType: 'gsd-executor', isolation: 'worktree' }), files_modified overlap -> separate sequential stages, resumeFromRunId wired to the phase run id, shared token budget) that composes the SAME gsd-executor agent and worktree isolation the inline path uses, restoring the wave parallelism the #853 backgrounded-agent nesting limitation forces inline on Hermes Agent. (The plan-checker and verifier remain inline until separately wired — this capability delivers the parallel-execution backend, not those gates.) Also folds the ultraplan plan-offload under one runtime gate (plan:* surface). On any runtime lacking the Workflow tool, or when the capability is disabled, behaviour is byte-identical to today (inline/manual dispatch). Detection + emission live in gsd-core/bin/lib/claude-orchestration.cjs (pure, fail-closed). Mirrors the existing gsd-ultraplan-phase BETA-isolation posture.",
    "tier": "full",
    "requires": [],
    "engines": {
      "gsd": ">=1.7.0"
  ...
  "external-job": {
    ...
    "config": {
      ...
      "external_job.submit_timeout_ms": {
        "type": "number",
        "default": 30000,
        "description": "Hard timeout (ms) for the scheduler submit subprocess (e.g. sbatch). Bounded per HERMES.md unbounded-subprocess policy. Read by the adapter (env GSD_SLURM_SUBMIT_TIMEOUT_MS overrides)."
      },
  ...
  "mempalace": {
    ...
    "config": {
      ...
      "mempalace.auto_capture_hooks": {
        "type": "boolean",
        "default": false,
        "description": "Reserved / not yet implemented: will install MemPalace's native stop/precompact Hermes Agent hooks for passive mid-session capture (the capability's hooks array is currently empty)."
      }
    },
    "steps": [
      {
  ...
  "profile-pipeline": {
    ...
    "description": "Developer behavioral profiling from Hermes Agent session history; scans session JSONL files, extracts and samples user messages, and generates profile artifacts (USER-PROFILE.md, dev-preferences.md, HERMES.md sections). Exposes eight `gsd-tools` commands: scan-sessions, extract-messages, profile-sample (pipeline phase) and write-profile, profile-questionnaire, generate-dev-preferences, generate-claude-profile, generate-claude-md (output phase). Backs the /gsd-profile-user skill and gsd-user-profiler agent.",
    "tier": "full",
    "requires": [],
    "engines": {
      "gsd": ">=1.6.0"

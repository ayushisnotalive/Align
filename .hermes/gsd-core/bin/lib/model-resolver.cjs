...
function warnModelOverrideUnmappable(agentType, overrideValue) {
    ...
    process.stderr.write(`gsd: warning — model_overrides value "${safe}" for ${agentType} is a fully-qualified ` +
        `Claude model ID with no tier alias; passing it through verbatim. Hermes Agent setups ` +
 ...
 * claude runtime, mirroring the `model_policy` path (#1144). Hermes Agent's
...
function resolveModelInternal(cwd, agentType) {
    ...
    if (tier && tier !== 'inherit') {
        ...
        if (policyModel) {
            ...
            if (!onClaude)
                return policyModel;
            // Hermes Agent's Agent tool takes tier aliases (opus/sonnet/haiku/fable),
            ...
            // than returning an ID Hermes Agent cannot spawn.

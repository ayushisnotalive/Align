...
""",
  gsd_role="gsd-debug-session-manager",
  gsd_role_prompt=<resolve "gsd-debug-session-manager" against the active install's gsd-* role prompts and load its contents; FAIL CLOSED with an explicit error if unresolved — never execute the role inline>,

...
Use `background=true` for all three to enable parallel execution.
...
```
delegate_task(
  gsd_role="gsd-doc-writer",
  gsd_role_prompt=<resolve "gsd-doc-writer" against the active install's gsd-* role prompts and load its contents; FAIL CLOSED with an explicit error if unresolved — never execute the role inline>,
...
> **ORCHESTRATOR RULE — CODEX RUNTIME**: After calling all Wave 1 delegate_task() calls above with `background=true`, do NOT generate any documentation independently while the subagents are active. Wait for all Wave 1 agents to complete before proceeding. This prevents duplicate work and wasted context.
...
Use `background=true` for all Wave 2 agents to enable parallel execution within the wave.
...
> **ORCHESTRATOR RULE — CODEX RUNTIME**: After calling all Wave 2 delegate_task() calls above with `background=true`, do NOT generate any documentation independently while the subagents are active. Wait for all Wave 2 agents to complete before proceeding. This prevents duplicate work and wasted context.

...
```
delegate_task(gsd_role="gsd-assumptions-analyzer", gsd_role_prompt=<resolve "gsd-assumptions-analyzer" against the active install's gsd-* role prompts and load its contents; FAIL CLOSED with an explicit error if unresolved — never execute the role inline>, role="leaf", prompt="""
...
```
delegate_task(gsd_role="general-purpose", gsd_role_prompt=<resolve "general-purpose" against the active install's gsd-* role prompts and load its contents; FAIL CLOSED with an explicit error if unresolved — never execute the role inline>, role="leaf", prompt="""

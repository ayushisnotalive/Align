...
opposite polarity, since firing concurrent delegate_task() dispatch is the risky direction here, not the
...
1. **Build the batch's plan-ID list** from the outline rows sharing this Wave value, in outline
   ...
   never produce two concurrent delegate_task() calls targeting the same `{plan_id}-PLAN.md` output path
...
3. Display:
   ...
   print one line per plan, immediately before that plan's own delegate_task() call, exactly as before.
   Concurrent dispatch: print one line per plan in the runnable set, immediately before issuing
   the batch's delegate_task() calls together.

4. Spawn the planner in **single-plan** mode — it must write exactly one PLAN.md file. The prompt
   is unchanged per plan; what changes is whether the runnable set's delegate_task() calls are issued one
   at a time (serial) or together in one message (concurrent, every call still carrying
   `background=true` exactly as today):
   ```javascript
   delegate_task(
     prompt="{same planning_context as step 8, plus:}
   ...
   ```
   **Serial dispatch:** issue one delegate_task() call, wait for it (step 5), verify and commit it
   (steps 6-7), THEN move to the next entry in the runnable set — byte-identical to the
   pre-#3777 loop.
   **Concurrent dispatch:** issue every runnable entry's delegate_task() call together, in this one

...
`background=true`, records `TS=$(date +%s)`, and then repeatedly
...
that prose-only surveillance, which cannot run during a *blocking* `delegate_task()`
...
`background=true` delegate_task() call's returned `async_launched` result,
...
delegate_task() return) — nothing else.
...
# delegate_task() call) or fresh mtime activity under $3 (an artifact glob), against

...
in Step 6's `delegate_task()` with `$HARNESS_FLAG`+comma when `ISOLATION = "harness-worktree"`, else
empty. `{harnessFlag}`
...
- ./HERMES.md or ./.hermes/HERMES.md (if exists — follow project-specific guidelines)
...
execute-phase's #683/#1369 guard). Hermes Agent's `isolation="worktree"` forks new worktrees from
...
- ${STATE_PATH} (Project state)
- ./HERMES.md or ./.hermes/HERMES.md (Project instructions, if exists)
...
",
  gsd_role="gsd-executor",
  gsd_role_prompt=<resolve "gsd-executor" against the active install's gsd-* role prompts and load its contents; FAIL CLOSED with an explicit error if unresolved — never execute the role inline>,
  role="leaf",  # nested delegation is unavailable at this dispatch depth/toolkit
  {harnessFlag}
...
delegate_task(
  ...
  Depth: quick",
  gsd_role="gsd-code-reviewer",
  gsd_role_prompt=<resolve "gsd-code-reviewer" against the active install's gsd-* role prompts and load its contents; FAIL CLOSED with an explicit error if unresolved — never execute the role inline>,
  role="leaf",  # nested delegation is unavailable at this dispatch depth/toolkit
)
```

...
<!-- gsd:protected -->
> **ORCHESTRATOR RULE — CODEX RUNTIME**: After calling all 4 researcher delegate_task() calls above, do NOT read research files or synthesize content independently while the subagents are active. Wait for all 4 researchers to complete before spawning the synthesizer. This prevents duplicate work and wasted context.
...
Use template: /home/ayushz/storage/CST studies/all projects/Align/.hermes/gsd-core/templates/research-project/SUMMARY.md
Commit after writing.
</output>
", gsd_role="gsd-research-synthesizer", gsd_role_prompt=<resolve "gsd-research-synthesizer" against the active install's gsd-* role prompts and load its contents; FAIL CLOSED with an explicit error if unresolved — never execute the role inline>, role="leaf", description="Synthesize research")
```
...
", gsd_role="gsd-roadmapper", gsd_role_prompt=<resolve "gsd-roadmapper" against the active install's gsd-* role prompts and load its contents; FAIL CLOSED with an explicit error if unresolved — never execute the role inline>, role="leaf", description="Create roadmap")
```
...
- Re-spawn roadmapper with revision context:
  ...
  ", gsd_role="gsd-roadmapper", gsd_role_prompt=<resolve "gsd-roadmapper" against the active install's gsd-* role prompts and load its contents; FAIL CLOSED with an explicit error if unresolved — never execute the role inline>, role="leaf", description="Revise roadmap")
  ```
...
- `.planning/ROADMAP.md`
- `.planning/STATE.md`
- `$INSTRUCTION_FILE` (runtime-derived via the shared `getProjectInstructionFile` policy: `AGENTS.md` for codex/opencode/kilo/kimi, `.github/copilot-instructions.md` for copilot, `GEMINI.md` for gemini/antigravity, `.hermes/HERMES.md` for claude)
...
- [ ] `$INSTRUCTION_FILE` generated with GSD workflow guidance (runtime-derived via the shared `getProjectInstructionFile` policy — `AGENTS.md` for codex/opencode/kilo/kimi, `.github/copilot-instructions.md` for copilot, `GEMINI.md` for gemini/antigravity, `.hermes/HERMES.md` for claude; an existing hand-crafted file without GSD markers is left untouched unless `--force`)

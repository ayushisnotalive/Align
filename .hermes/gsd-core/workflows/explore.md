...
@/home/ayushz/storage/CST studies/all projects/Align/.hermes/gsd-core/references/questioning.md
...
guard below. `--pick model` answers a different question — which model id to hand `delegate_task()` — and is
...
delegate_task(
  ...
  gsd_role="gsd-phase-researcher",
  gsd_role_prompt=<resolve "gsd-phase-researcher" against the active install's gsd-* role prompts and load its contents; FAIL CLOSED with an explicit error if unresolved — never execute the role inline>,

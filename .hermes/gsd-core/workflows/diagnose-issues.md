...
Hermes Agent's `isolation="worktree"` forks new worktrees from `origin/HEAD`, not the live local
...
**Substitute `{harnessFlag}` in the `delegate_task()` call below** with `$HARNESS_FLAG` followed by a
...
delegate_task(
  ...
  gsd_role="gsd-debugger",
  gsd_role_prompt=<resolve "gsd-debugger" against the active install's gsd-* role prompts and load its contents; FAIL CLOSED with an explicit error if unresolved — never execute the role inline>,

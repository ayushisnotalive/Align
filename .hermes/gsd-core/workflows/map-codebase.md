...
<step name="spawn_agents" condition="delegate_task is available">
...
Use delegate_task with `gsd_role="gsd-codebase-mapper"`, `background=true` for parallel execution.
...
```text
delegate_task(
  gsd_role="gsd-codebase-mapper",
  gsd_role_prompt=<resolve "gsd-codebase-mapper" against the active install's gsd-* role prompts and load its contents; FAIL CLOSED with an explicit error if unresolved — never execute the role inline>,
...
<step name="sequential_mapping" condition="delegate_task is NOT available (e.g. Antigravity, Gemini CLI, Codex)">
...
- If delegate_task available: 4 parallel gsd-codebase-mapper agents spawned with background=true
- If delegate_task NOT available: 4 sequential mapping passes performed inline (never using browser_subagent)

...
3. **Create worktrees + dispatch executors, ONE AT A TIME per `spawn` item**
   ...
   **`isolation == "harness-worktree"`:** one `delegate_task()` per message,
   `background: true`. Same prompt shape as `/gsd-quick`'s own
   ...
   - ./HERMES.md or ./.hermes/HERMES.md (if exists)
   </required_reading>
   ...
   **Durable worktree-recovery persistence (#3677 — `harness-worktree`/
   ...
   > **ORCHESTRATOR RULE — CODEX RUNTIME**: after each `delegate_task()` call above, wait for it to return before starting the next worktree create.

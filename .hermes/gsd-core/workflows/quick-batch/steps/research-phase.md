...
Dispatch one `delegate_task()` PER MESSAGE, `background: true`, up to
`$RESEARCH_CONCURRENCY` in flight at once — never multiple `delegate_task()` calls in
...
- ./HERMES.md or ./.hermes/HERMES.md (if exists — project-specific guidelines)
</required_reading>

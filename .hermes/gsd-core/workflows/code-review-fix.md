...
Use delegate_task() to spawn agent:
...
```text
delegate_task(gsd_role="gsd-code-fixer", gsd_role_prompt=<resolve "gsd-code-fixer" against the active install's gsd-* role prompts and load its contents; FAIL CLOSED with an explicit error if unresolved — never execute the role inline>, role="leaf", prompt="
<required_reading>
${REVIEW_PATH}
</required_reading>
...
if [ "$AUTO_MODE" = "true" ]; then
  ...
  - ${f}"
    ...
    delegate_task(gsd_role="gsd-code-reviewer", gsd_role_prompt=<resolve "gsd-code-reviewer" against the active install's gsd-* role prompts and load its contents; FAIL CLOSED with an explicit error if unresolved — never execute the role inline>, role="leaf", prompt="
<config>
depth: ${REVIEW_DEPTH}
phase_dir: ${PHASE_DIR}
review_path: ${REVIEW_PATH}
...
${AGENT_SKILLS_REVIEWER}")
    # ORCHESTRATOR RULE — CODEX RUNTIME: After calling delegate_task() above, stop working on this task immediately. Do not read more files, edit code, or run tests related to this task while the subagent is active. Wait for the subagent to return its result before proceeding.

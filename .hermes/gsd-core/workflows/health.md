...
the former — telling a Hermes Agent user their runtime declares no executor-isolation primitive,
...
TASKS_DIR="/home/ayushz/storage/CST studies/all projects/Align/.hermes/tasks"
if [ -d "$TASKS_DIR" ]; then
  STALE_COUNT=$( (find "$TASKS_DIR" -maxdepth 1 -type d -mtime +1 2>/dev/null || true) | wc -l )
  if [ "$STALE_COUNT" -gt 0 ]; then
    echo "⚠️  Found $STALE_COUNT stale task directories in /home/ayushz/storage/CST studies/all projects/Align/.hermes/tasks/"
    echo "   These are leftover from crashed subagent sessions."
    echo "   Run: rm -rf /home/ayushz/storage/CST studies/all projects/Align/.hermes/tasks/*  (safe — only affects dead sessions)"
  fi
fi
```

...
**GSD Core** (Git. Ship. Done.) creates hierarchical project plans optimized for solo agentic development with Hermes Agent.
...
**`/gsd-spike --wrap-up`** — Curates spikes one-at-a-time (include/exclude/partial/UAT), generates a project skill under `./.hermes/skills/spike-findings-[project]/`, writes `.planning/spikes/WRAP-UP-SUMMARY.md`, adds a HERMES.md auto-load line.

Usage: `/gsd-spike --wrap-up`

**`/gsd-sketch --wrap-up`** — Same curation flow for sketches, generating `./.hermes/skills/sketch-findings-[project]/` with design decisions/CSS/HTML structures.
...
**`/gsd-capture --note <text>`** — Zero-friction timestamped note to `.planning/notes/` (or `/home/ayushz/storage/CST studies/all projects/Align/.hermes/notes/` globally). Subcommands: append (default), list, promote (note → todo). Works without a project.
...
- **`/gsd-ultraplan-phase [phase]`** — [BETA] Offload plan phase to Hermes Agent's ultraplan cloud; review in browser, import back.

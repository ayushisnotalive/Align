...
Run for each plan, immediately before its `delegate_task()` dispatch in step 3. Sets
...
- The resolved `EXECUTOR_TYPE` is used as `subagent_type` in BOTH worktree and
  sequential dispatch (sequential reuses the worktree-mode `delegate_task()` template).
...
# #1689 v1 routes only the delegate_task()-based dispatch. On the orchestrator-worktree
...
Routing applies to the `delegate_task()`-based dispatch (harness-worktree and sequential
...
When composing the `delegate_task()` prompt, do NOT add text refusing or overriding

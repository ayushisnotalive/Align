...
Substitute `$HARNESS_FLAG`'s value for the `{harnessFlag}` placeholder in the `delegate_task()` dispatch
in `execute-phase.md` step 3 (on Hermes Agent it is literally `isolation="worktree"`).
...
#      harness path substitutes into its delegate_task() prompt). {plan_id} is this
...
<mcp_tools>
If HERMES.md or project instructions reference MCP tools, prefer those tools
...
WT_PATH="${ORCH_ROOT}/.hermes/worktrees/${AGENT_ID}"

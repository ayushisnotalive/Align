...
**Model on every classifier spawn (#3602):** is a parameter of each Task/Agent call — not a prompt field, never folded into the prompt text — so `dynamic_routing`/`model_profile` tiers apply instead of the caller's session model. Omit the parameter per the rule above when the value is `"inherit"` or empty.
...
```
delegate_task(
  gsd_role="gsd-doc-synthesizer",
  gsd_role_prompt=<resolve "gsd-doc-synthesizer" against the active install's gsd-* role prompts and load its contents; FAIL CLOSED with an explicit error if unresolved — never execute the role inline>,
...
```
delegate_task(
  gsd_role="gsd-roadmapper",
  gsd_role_prompt=<resolve "gsd-roadmapper" against the active install's gsd-* role prompts and load its contents; FAIL CLOSED with an explicit error if unresolved — never execute the role inline>,
  role="leaf",  # nested delegation is unavailable at this dispatch depth/toolkit
  prompt: "

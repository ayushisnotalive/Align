...
@/home/ayushz/storage/CST studies/all projects/Align/.hermes/gsd-core/references/ui-brand.md
@/home/ayushz/storage/CST studies/all projects/Align/.hermes/gsd-core/references/revision-loop.md
@/home/ayushz/storage/CST studies/all projects/Align/.hermes/gsd-core/references/gate-prompts.md
...
Claude Code has no `Agent` tool. It exposes `delegate_task`, which dispatches by structural role — it has no concept of a named subagent identity. GSD projects each named gsd-* role onto this primitive itself: resolve the role's prompt file from the active install, load its contents, and embed them in the `delegate_task` payload via `gsd_role_prompt` as the dispatched task's operating instructions. FAIL CLOSED — surface an explicit error and stop — if a referenced role prompt cannot be resolved; never execute the role inline as a substitute. In a top-level Claude Code session, always dispatch gsd-phase-researcher, gsd-planner, and gsd-plan-checker as separate `delegate_task` calls.
...
delegate_task() call for gsd-phase-researcher, gsd-planner, and gsd-plan-checker. Only
a real tool-unavailable error returned by delegate_task() is a reliable absence signal —
...
**#2517:** omit the `model=` param from an `delegate_task()` call when its `researcher`/`planner`/`checker`_model is `"inherit"` or empty — passing 404s on non-Claude runtimes; omitting inherits the orchestrator model (mirrors execute-phase).
...
- Planner is instructed to produce `SKELETON.md` in the phase directory alongside `PLAN.md`. The template lives at `/home/ayushz/storage/CST studies/all projects/Align/.hermes/gsd-core/references/skeleton-template.md` — the planner reads it when producing SKELETON.md (lazy; not loaded on non-skeleton runs).
...
delegate_task(
  ...
  gsd_role=research_hook.ref.agent,
  gsd_role_prompt=<resolve research_hook.ref.agent against the active install's gsd-* role prompts and load its contents; FAIL CLOSED with an explicit error if unresolved — never execute the role inline>,
...
**If found:**
1. Read template: `/home/ayushz/storage/CST studies/all projects/Align/.hermes/gsd-core/templates/VALIDATION.md`
...
- If `ref.agent` is set, dispatch with `delegate_task(prompt=filled_hook_fragment, gsd_role=ref.agent, gsd_role_prompt=<resolve ref.agent against the active install's gsd-* role prompts and load its contents; FAIL CLOSED with an explicit error if unresolved — never execute the role inline>, role="leaf")`. Use the hook's `fragment.inline` as the prompt body and fill phase fields before spawning.
...
delegate_task(
  ...
  gsd_role=pattern_mapper_hook.ref.agent,
  gsd_role_prompt=<resolve pattern_mapper_hook.ref.agent against the active install's gsd-* role prompts and load its contents; FAIL CLOSED with an explicit error if unresolved — never execute the role inline>,
  role="leaf",  # nested delegation is unavailable at this dispatch depth/toolkit
)
```
...
**Read and run** the gate + edge probe in `/home/ayushz/storage/CST studies/all projects/Align/.hermes/gsd-core/references/specless-probe-fallback.md`
...
**Project instructions:** Read ./HERMES.md or ./.hermes/HERMES.md if either exists — follow project-specific guidelines
...
**MVP_MODE:** ${MVP_MODE} (when true, follow vertical-slice rules from `/home/ayushz/storage/CST studies/all projects/Align/.hermes/gsd-core/references/planner-mvp-mode.md`; when false, ignore MVP guidance entirely.)
**WALKING_SKELETON:** ${WALKING_SKELETON} (when true, the first deliverable must be a Walking Skeleton — Read the template at `/home/ayushz/storage/CST studies/all projects/Align/.hermes/gsd-core/references/skeleton-template.md` and produce SKELETON.md alongside PLAN.md.)
**Granularity:** {granularity}

${MVP_MODE === 'true' ? `
<mvp_mode_active>
**MVP Mode is ENABLED.** Read `/home/ayushz/storage/CST studies/all projects/Align/.hermes/gsd-core/references/planner-mvp-mode.md` now and follow its vertical-slice planning rules. Each plan must deliver a complete vertical slice — thin end-to-end functionality rather than horizontal layers.
</mvp_mode_active>
` : ''}
...
else-branch below, per §A/§B/§C of `/home/ayushz/storage/CST studies/all projects/Align/.hermes/gsd-core/references/specless-probe-fallback.md`
...
delegate_task(
  ...
  gsd_role_prompt=<resolve "gsd-planner" against the active install's gsd-* role prompts and load its contents; FAIL CLOSED with an explicit error if unresolved — never execute the role inline>,
  role="leaf",  # nested delegation is unavailable at this dispatch depth/toolkit
...
**If `CHUNKED_MODE` is `true`:** Skip the delegate_task() call above — proceed to step 8.5 instead.
...
**Triggered when:** delegate_task() returns but the return contains no recognized marker (`## PLANNING COMPLETE`, `## PHASE SPLIT RECOMMENDED`, `## ⚠ Source Audit`, `## CHECKPOINT REACHED`, `## PLANNING INCONCLUSIVE`).
...
**Project instructions:** Read ./HERMES.md or ./.hermes/HERMES.md if either exists — verify plans honor project guidelines
...
delegate_task(
  ...
  gsd_role_prompt=<resolve "gsd-plan-checker" against the active install's gsd-* role prompts and load its contents; FAIL CLOSED with an explicit error if unresolved — never execute the role inline>,
  role="leaf",  # nested delegation is unavailable at this dispatch depth/toolkit
...
**Triggered when:** Checker delegate_task() returns but the return contains neither `## VERIFICATION PASSED` nor `## ISSUES FOUND`.

...
Resolve each agent's model through `gsd-tools`, then pass it to the `delegate_task()` spawn — or
...
@/home/ayushz/storage/CST studies/all projects/Align/.hermes/gsd-core/references/model-profiles.md
...
delegate_task(
  prompt="...",
  gsd_role="gsd-planner", gsd_role_prompt=<resolve "gsd-planner" against the active install's gsd-* role prompts and load its contents; FAIL CLOSED with an explicit error if unresolved — never execute the role inline>, role="leaf"
)
```
...
**#2684 — substitute a field your workflow actually bound.** must name a key
...
3. Pass on each `delegate_task()` spawn

...
<project_context>
Read `./HERMES.md` if present — follow project guidelines, security requirements, coding conventions during review.

**Project skills:** check `.hermes/skills/` or `.agents/skills/`: list skill subdirectories, read each `SKILL.md` (lightweight index ~130 lines), load specific `rules/*.md` as needed. Do NOT load full `AGENTS.md` files (100KB+ context cost). Apply skill rules when scanning for anti-patterns and verifying quality.
...
**DO consider project conventions** from HERMES.md — a violation in one project may be standard in another.

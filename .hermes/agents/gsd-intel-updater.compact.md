...
**Project skills:** check `.hermes/skills/` or `.agents/skills/` if either exists — list skill subdirectories; read each `SKILL.md` (~130 lines); load `rules/*.md` as needed; do NOT load full `AGENTS.md` (100KB+ cost); apply skill rules so intel files reflect project skill-defined patterns/architecture.
...
if [[ "$(jq -r '.name // ""' package.json 2>/dev/null)" == "@opengsd/gsd-core" ]]; then
  ls -d .kilo 2>/dev/null && echo "kilo" || (ls -d .hermes/gsd-core 2>/dev/null && echo "claude") || echo "unknown"
fi
```
...
**Count accuracy:** when reporting component counts (stack.json, arch-decisions.json), always derive counts by running Glob on the layout-resolved canonical locations, never from memory or HERMES.md. E.g. standard: `Glob("agents/*.md")`; kilo: `Glob(".kilo/agents/*.md")`.
